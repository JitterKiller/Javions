...
public final class AircraftDatabase {
    ...
    public AircraftData get(IcaoAddress address) throws IOException {
        String database = Objects.requireNonNull(getClass().getResource("/aircraft.zip")).getFile();
                ...
                if (!entry.isDirectory() && entry.getName().endsWith(".csv")) {
                    try (BufferedReader reader = new BufferedReader(new InputStreamReader(zipFile.getInputStream(entry)))) {
                        ...
                        while ((line = reader.readLine()) != null) {
                            ...
                            if (currentAddress.OACI().compareTo(address.OACI()) == 0) {
                            ...
                            } else if (currentAddress.OACI().compareTo(address.OACI()) > 0) {
                                break;
                            }
                        }
                    } catch (IOException e) {
                        throw new IOException("Error while reading file "+ entry.getName());
                    }
                }
            }
        } catch (IOException e){
            throw new IOException("Error while opening zip file", e);
        }
        return null;
    }
}
